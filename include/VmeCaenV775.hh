...
class VmeCaenV775 : public VmeModule
{
public:
  explicit VmeCaenV775( const std::string& module_name,
			const unsigned int base_address );
  ~VmeCaenV775( void );

private:
  VmeCaenV775( const VmeCaenV775& );
  VmeCaenV775& operator =( const VmeCaenV775& );

private:
  const double k_LSB = 0.035; // [ns]
  // general
...
private:
  ...
  unsigned int m_valid[k_n_channel];
  unsigned int m_tdc[k_n_channel];

public:
  bool     Clear( void );
  double   GetLSB( void ) const { return k_LSB; }
